    ...
    <div class="input-text">
        ...
        <div class="input-wrapper">
            ...
            <input type="text" :class="{ error: isZerro}" v-model="ipt" 
                @input="inputHandler" @focus="setFocus"
                @mouseup="setFocusEnd">
        </div>
    </div>
</template>
...
export default {
    ...
    setup(props, {emit}) {
        ...
        const isZerro = computed(() => {
            if(Number(ipt.value) === 0) return true
            return false
        })
        const setFocus = ev => {
            ev.preventDefault()
        ...
        const inputHandler = () => {
            if(icon.value === 'money') {
                ...
                .replace(/[^\d\.]/, '')
                .replace(/^\./, '0.')
                .replace(/\.\./, '.').replace(/(\.\d)\./, '$1').replace(/(\.\d\d)./, '$1')
            }
            if(icon.value === 'person') {
                ipt.value = Number(ipt.value.replace(/\D/, '')).toString()
            }
            emit('inputChange', ipt.value)
        }
        return {
            ipt, inputHandler, isZerro, setFocus, setFocusEnd
        }
    }
}
</script>
